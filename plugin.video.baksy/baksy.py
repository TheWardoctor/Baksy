...
import base64, time
import unicodedata
AddonID = 'plugin.video.baksy'
ADDON=xbmcaddon.Addon(id='plugin.video.baksy')
__icon__        = ADDON.getAddonInfo('icon')
__self__        = xbmcaddon.Addon(id='plugin.video.baksy')
...
def correctPVR():
	...
	flawiptv = xbmcaddon.Addon('plugin.video.baksy')
	...
	dialog.ok("Baksy IPTV",'IPTV Simple Client set-up is complete.',' ','Please allow it time to load the EPG to avoid issues.')
...
def speed():
	xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.baksy/speedtest.py")')
	
def guide():
	xbmc.executebuiltin('Runscript("special://home/addons/script.baksy.guide/addon.py")')
