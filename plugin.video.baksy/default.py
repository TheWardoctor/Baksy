...
def peamenyy(params):
    ...
    channels = kontroll()
    if channels == 1 and baksy.mode != 5 and baksy.mode != 1:
        plugintools.log(pnimi+vod_channels("TG9naW4gU3VjY2VzcyAtIFdhcmRvY3RvciBXaW5zIQ=="))
        plugintools.addItem('[B]Baksy IPTV Guide[/B]','guide',8,baksy.Images + 'baksy.png',baksy.Images + 'background.png')
        ...
        if not xbmc.getCondVisibility('System.HasAddon(pvr.iptvsimple)'):
            plugintools.addItem('[B]Setup PVR Client with Baksy IPTV[/B]','pvr',10,baksy.Images + 'pvr.png',baksy.Images + 'background.png')
        else:
            plugintools.addItem('[B]Launch PVR Baksy IPTV[/B]','pvr',11,baksy.Images + 'pvr.png',baksy.Images + 'background.png')
        plugintools.addItem('[B]Run Speedtest[/B]','speed',9,baksy.Images + 'speed.png',baksy.Images + 'background.png')
        baksy.AddDir('[B][COLOR=red]Clear Cache[/B][/COLOR]','Clear Cache',7,baksy.Images + 'cache.png')
    elif channels != 1 and baksy.mode != 1:
        plugintools.add_item( action=vod_channels("bGljZW5zZV9jaGVjaw=="), title="[B]Step 1. Enter Your Baksy IPTV Login Credentials.[/B]" , thumbnail=os.path.join(LOAD_LIVE,vod_channels("c2V0dGluZ3MucG5n")), folder=False )	
