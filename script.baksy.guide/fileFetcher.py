...
class FileFetcher(object):
    ...
    basePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.baksy.guide'))
    filePath = ''
    fileUrl = ''
    addon = None
    ...
    def fetchFile(self,force=False):
        ...
        if fetch:
            ...
            if self.fileType == self.TYPE_DEFAULT:
                xbmc.log('[script.baksy.guide] file is in remote location: %s' % self.fileUrl, xbmc.LOGDEBUG)
                ...
                if not xbmcvfs.copy(self.fileUrl, tmpFile):
                    xbmc.log('[script.baksy.guide] Remote file couldn\'t be copied: %s' % self.fileUrl, xbmc.LOGERROR)
            else:
                ...
                if self.addon.getSetting('md5') == 'true':
                    ...
                    try:
                        ...
                        xbmc.log('[script.baksy.guide] Missing md5: %s.md5 (%s)' % (self.fileUrl,detail), xbmc.LOGERROR)
                    #log((old_md5,new_md5))
                    if old_md5 and (old_md5 == new_md5) and (self.addon.getSetting('xmltv.refresh') == 'false'):
                        return self.FETCH_NOT_NEEDED
                f = open(tmpFile, 'wb')
                xbmc.log('[script.baksy.guide] file is on the internet: %s' % self.fileUrl, xbmc.LOGDEBUG)
                total = 0
                ...
                try:
                    ...
                    if r.status_code != requests.codes.ok:
                        if ADDON.getSetting('gz') == 'true':
                            ...
                            if r.status_code != requests.codes.ok:
                                xbmc.log('[script.baksy.guide] no file: %s' % self.fileUrl, xbmc.LOGERROR)
                                xbmcgui.Dialog().notification("Baksy IPTV Guide", "bad status code %s" % self.fileUrl,xbmcgui.NOTIFICATION_ERROR)
                        else:
                            xbmc.log('[script.baksy.guide] no file: %s' % fileUrl, xbmc.LOGERROR)
                            xbmcgui.Dialog().notification("Baksy IPTV Guide", "bad status code %s " % fileUrl,xbmcgui.NOTIFICATION_ERROR)
                            return self.FETCH_NOT_NEEDED
                    if "Content-Length" in r.headers:
                        total = int(r.headers['Content-Length'])
                except Exception as detail:
                    xbmc.log('[script.baksy.guide] bad request: %s (%s)' % (fileUrl,detail), xbmc.LOGERROR)
                    xbmcgui.Dialog().notification("Baksy IPTV Guide", "failed to download %s " % fileUrl,xbmcgui.NOTIFICATION_ERROR)
                    return self.FETCH_NOT_NEEDED

                title = fileUrl.split('/')[-1]
                d = xbmcgui.DialogProgressBG()
                d.create('Baksy IPTV Guide', 'downloading %s' % title)
                chunk_size = 16 * 1024
                size = 0
                oldtime = time.time()
            ...
            if new_md5 and (self.addon.getSetting('md5') == 'true'):
                ...
                if md5_file != new_md5:
                    xbmc.log('[script.baksy.guide] md5 mismatch: %s calculated:%s server:%s' % (self.fileUrl,md5_file,new_md5), xbmc.LOGERROR)
                    xbmcgui.Dialog().notification("Baksy IPTV Guide", "failed md5 check %s",xbmcgui.NOTIFICATION_ERROR)
                else:
                    xbmcvfs.File(self.filePath+".md5","wb").write(new_md5)
            retVal = self.FETCH_OK
            xbmc.log('[script.baksy.guide] file %s was downloaded' % self.filePath, xbmc.LOGDEBUG)
        return retVal