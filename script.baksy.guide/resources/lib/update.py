...
def check_update():
	text_file = xbmc.translatePath('special://home/addons/script.baksy.guide/resources/sfhash.txt')
	HTML = OPEN_URL('https://github.com/Baksy IPTV GuideTV/addons/blob/master/Baksy IPTV Guide.zip')
	regex = re.compile('Baksy IPTV GuideTV/addons/commit/(.+?)" class="message').findall(HTML)
	for commit in regex:
		text = commit
		...
		if text == text_file_contents:
			...
			text_file.close()
			print commit
			xbmc.executebuiltin('RunScript(special://home/addons/script.baksy.guide/resources/lib/updateSF.py)')
