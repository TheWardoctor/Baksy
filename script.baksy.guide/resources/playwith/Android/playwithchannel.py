...
path = xbmc.translatePath('special://profile/addon_data/script.baksy.guide/source.db')
try:
    conn = sqlite3.connect(path, detect_types=sqlite3.PARSE_DECLTYPES)
    conn.row_factory = sqlite3.Row
except Exception as detail:
    xbmc.log("EXCEPTION: (script.baksy.guide)  %s" % detail, xbmc.LOGERROR)
...
if url:
    ...
    dialog.notification("Baksy IPTV Guide","Starting Recording",sound=True)
    ...
    dialog.notification("Baksy IPTV Guide","Finished Recording",sound=True)