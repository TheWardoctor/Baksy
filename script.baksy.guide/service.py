...
class Service(object):
    ...
    def onInit(self, success):
        if success:
            xbmc.log("[script.baksy.guide] Background Update Starting...", xbmc.LOGNOTICE)
...
if __name__ == '__main__':
    ADDON = xbmcaddon.Addon('script.baksy.guide')

    version = ADDON.getAddonInfo('version')
    if ADDON.getSetting('version') != version:
        #text = xbmcvfs.File('special://home/addons/script.baksy.guide/changelog.txt','rb').read()
        #xbmcgui.Dialog().textviewer("Baksy IPTV Guide",text)
    ...
    xbmcvfs.delete('special://profile/addon_data/script.baksy.guide/source.db-journal')
    lock = 'special://profile/addon_data/script.baksy.guide/db.lock'
    xbmcvfs.delete(lock)
    try:
        if ADDON.getSetting('autostart') == "true":
            xbmc.executebuiltin("RunAddon(script.baksy.guide)")

        if ADDON.getSetting('background.service') == 'true':
            monitor = xbmc.Monitor()
            xbmc.log("[script.baksy.guide] Background service started...", xbmc.LOGDEBUG)
            if ADDON.getSetting('background.startup') == 'true':
                ...
                if ADDON.getSetting('service.addon.folders') == "true":
                    xbmc.executebuiltin('RunScript(special://home/addons/script.baksy.guide/ReloadAddonFolders.py)')
            while not monitor.abortRequested():
                if ADDON.getSetting('service.type') == '0':
                    ...
                    xbmc.log("[script.baksy.guide] Service waiting for interval %s" % waitTime, xbmc.LOGDEBUG)
                else:
                ...
                if timeLeft < 0:
                    timeLeft = 0
                xbmc.log("[script.baksy.guide] Service waiting for %d seconds" % timeLeft, xbmc.LOGDEBUG)
                if timeLeft and monitor.waitForAbort(timeLeft):
                    break
                xbmc.log("[script.baksy.guide] Service now triggered...", xbmc.LOGDEBUG)
                Service()
                if ADDON.getSetting('service.addon.folders') == "true":
                    xbmc.executebuiltin('RunScript(special://home/addons/script.baksy.guide/ReloadAddonFolders.py)')
                now = time.time()
        ...
        xbmc.log('[script.baksy.guide] Uncaught exception in service.py: %s' % str(ex), xbmc.LOGDEBUG)
